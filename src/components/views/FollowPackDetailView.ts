...
export class FollowPackDetailView extends View {
  ...
  private async render(): Promise<void> {
    ...
    try {
      ...
      if (!event) {
        this.container.innerHTML =
          '<div class="article-view-error"><p>Follow pack not found</p></div>';
        return;
      }
  ...
  private async renderPack(event: NostrEvent): Promise<void> {
    ...
    const authorHeaderMount = this.container.querySelector(
      '.follow-packs__detail-author-header'
    );
    ...
    const actionsContainer = this.container.querySelector(
      '.follow-packs__detail-actions'
    );
    if (actionsContainer && isLoggedIn) {
      const { isFollowPacksEnabled } = await import(
        '../../addons/follow-packs/index'
      );
  ...
  private setupActions(pack: FollowPack): void {
    ...
    this.container
      ...
      ?.addEventListener('click', async () => {
        ...
        const newFollows = pack.userPubkeys.filter(
          pk => !followedPubkeys.has(pk)
        );
        ...
        const { ToastService } = await import('../../services/ToastService');
        ToastService.show(
          `Following ${newFollows.length} new people`,
          'success'
        );
      });
    ...
    this.container
      ...
      ?.addEventListener('click', () => {
        TypedEventBus.getInstance().emit('list:open', {
          listType: 'follow-packs',
          packId: pack.id,
          packMode: 'timeline' as const,
        });
      });
    ...
    this.container
      ...
      ?.addEventListener('click', () => {
        TypedEventBus.getInstance().emit('list:open', {
          listType: 'follow-packs',
          packId: pack.id,
          packMode: 'edit' as const,
        });
      });
  }

  private async loadMembers(pack: FollowPack): Promise<void> {
    const memberList = this.container.querySelector(
      '.follow-packs__members'
    ) as HTMLElement | null;
