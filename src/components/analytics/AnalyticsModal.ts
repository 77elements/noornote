...
export class AnalyticsModal {
  ...
  public async show(noteId: string, rawEvent?: NostrEvent): Promise<void> {
    ...
    try {
      ...
      if (!stats) {
        this.modalService.show({
          ...MODAL_CONFIG,
          content: this.renderErrorContent('Reactions module not available'),
        });
        return;
      }
      const statsContent = await this.renderStatsContent(
        stats,
        noteId,
        rawEvent
      );
      ...
      this.modalService.show({
        ...MODAL_CONFIG,
        content: this.renderErrorContent('Failed to load analytics data'),
      });
    }
  }
  ...
  private renderSection(
    title: string,
    content: string,
    listClass = 'analytics-modal__list'
  ): string {
    return `
      <div class="analytics-modal__section">
        <h2>${title}</h2>
  ...
  private getProfile(
    pubkey: string,
    profileMap: Map<string, UserMentionProfile>
  ): UserMentionProfile {
  ...
  private async renderStatsContent(
    ...
    const repliesSection = this.renderRepliesSection(
      stats.replyEvents,
      profileMap
    );
    const zapsSection = this.renderZapsSection(stats.zapEvents, profileMap);
    const repostsSection = this.renderRepostsSection(
      stats.repostEvents,
      profileMap
    );
    const quotedSection = this.renderQuotedRepostsSection(
      stats.quotedEvents,
      profileMap
    );
    ...
    const clientTag = rawEvent?.tags?.find(
      (tag: string[]) => tag[0] === 'client'
    );
    const clientName = clientTag?.[1] || null;
    const clientSection = clientName
      ? `<div class="analytics-modal__client">via ${escapeHtml(clientName)}</div>`
      : '';
  ...
  private renderZapsSection(
    ...
    const zapItems = zapEvents
      .map(event => {
        ...
        const messageHtml = zapMessage
          ? ` <span class="analytics-modal__zap-message">(${escapeHtml(zapMessage)})</span>`
          : '';
    ...
    return this.renderSection(
      `Zaps (${zapEvents.length}): ${formatNumberWithCommas(totalSats)} Sats`,
      zapItems,
      'analytics-modal__zap-list'
    );
  }
   ...
   */
  private renderQuotedRepostsSection(
    ...
    if (quotedEvents.length === 0) {
      return this.renderEmptySection(
        'Quoted Reposts (0)',
        'No quoted reposts yet'
      );
    }

    const userLinks = quotedEvents
      .map(event => {
        const profile = this.getProfile(event.pubkey, profileMap);
        return this.renderNoteLink(event, profile);
      })
    ...
    return this.renderSection(
      `Quoted Reposts (${quotedEvents.length})`,
      userLinks
    );
  }
  ...
  private buildLikesTree(
    ...
    for (const reaction of reactionEvents) {
      ...
      body.appendChild(
        this.buildTopLevelReactionRow(reaction, noteId, rawEvent, ctx)
      );
    }
  }
  ...
  private buildTopLevelReactionRow(
    ...
    const noteAuthor =
      rawEvent?.pubkey || reaction.tags.find(t => t[0] === 'p')?.[1] || '';
    row.appendChild(
      buildEmojiMenu(
        reaction,
        noteId,
  ...
  private updateISLInDOM(noteId: string, stats: DetailedStats): void {
    ...
    const islContainer = document.querySelector(
      `.isl[data-note-id="${noteId}"]`
    );
    if (!islContainer) {
      return;
    }
    ...
    const quotedRepostsCount = islContainer.querySelector(
      '.isl-quote .isl-count'
    );
    ...
    if (repliesCount) {
      repliesCount.textContent = this.formatCountShort(
        stats.replyEvents.length
      );
    }
    if (repostsCount) {
      repostsCount.textContent = this.formatCountShort(
        stats.repostEvents.length
      );
    }
    if (quotedRepostsCount) {
      quotedRepostsCount.textContent = this.formatCountShort(
        stats.quotedEvents.length
      );
    }
    if (likesCount) {
      likesCount.textContent = this.formatCountShort(
        stats.reactionEvents.length
      );
    }
    if (zapsCount) {
