...
function updateLastModified(data: BookmarkSetData): void {
  data.metadata.lastModified = nowTimestamp();
}
...
export function createEmptyBookmarkSetData(): BookmarkSetData {
  return {
    version: 2,
    sets: [createEmptySet('')],
    metadata: { setOrder: [''], lastModified: nowTimestamp() }
  };
}
...
export function toNostrEvents(
  ...
  return Promise.all(data.sets.map(async (set) => {
    ...
    const content = set.privateTags.length > 0
      ? await encryptPrivateTags(set.privateTags, pubkey)
      : '';
...
export function fromNostrEvents(
  ...
  return (async () => {
    ...
    for (const [dTag, event] of eventsByDTag) {
      ...
      sets.push({ kind: 30003, d: dTag, title: titleTag, publicTags, privateTags });
    }

    if (!setOrder.includes('')) {
      setOrder.unshift('');
      sets.unshift(createEmptySet(''));
    }
...
export function addBookmark(data: BookmarkSetData, dTag: string, tag: BookmarkTag, isPrivate: boolean): void {
