...
export class SettingsRuntime implements ModuleRuntime<SettingsModuleApi> {
  private relayListOrch:
    | import('../../services/orchestration/RelayListOrchestrator').RelayListOrchestrator
    | null = null;
  private RelayListOrchClass:
    | typeof import('../../services/orchestration/RelayListOrchestrator').RelayListOrchestrator
    | null = null;
  private healthMonitor:
    | import('../../services/RelayHealthMonitor').RelayHealthMonitor
    | null = null;
  private updateCheckService:
    | import('../../services/UpdateCheckService').UpdateCheckService
    | null = null;
  ...
  getApi(): SettingsModuleApi {
    ...
    return {
      fetchRelayList: (pubkey, bootstrapRelays) =>
        rlo?.fetchRelayList(pubkey, bootstrapRelays) ?? Promise.resolve(null),
      publishRelayList: (relays, event) =>
        rlo?.publishRelayList(relays, event) ?? Promise.resolve(),
      relayInfosToTags: relays => RloCls?.relayInfosToTags(relays) ?? [],
      getRelayHealthMetrics: url => hm?.getMetrics(url) ?? null,
      getHealthSummary: () =>
        hm?.getHealthSummary() ??
        Promise.resolve({ healthy: 0, total: 0, warnings: [] }),
      ...
      checkUpdateManually: button =>
        ucs?.checkManually(button) ?? Promise.resolve(),
    };
  }
}
