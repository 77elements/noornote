...
export class ReactionsRuntime implements ModuleRuntime<ReactionsModuleApi> {
  private orchestrator:
    | import('../../services/orchestration/ReactionsOrchestrator').ReactionsOrchestrator
    | null = null;
  private reactionService:
    | import('../../services/ReactionService').ReactionService
    | null = null;
  private statsUpdateService:
    | import('../../services/StatsUpdateService').StatsUpdateService
    | null = null;
  ...
  getApi(): ReactionsModuleApi {
    ...
    return {
      getStats: (noteId, authorPubkey, eventId) =>
        orch?.getStats(noteId, authorPubkey, eventId) ??
      ...
      hasUserLikedWithEmoji: (noteId, emoji) =>
        svc?.hasUserLikedWithEmoji(noteId, emoji) ?? Promise.resolve(false),
      ...
      updateAfterInteraction: (noteId, type, islComponent) =>
        sus?.updateAfterInteraction(noteId, type, islComponent),
