...
export class FollowerNotificationService {
  ...
  public async start(): Promise<void> {
    ...
    this.systemLogger.info('FollowerNotificationService', 'Started — first check in 3 min, then every 3 h');
  ...
  private async tick(): Promise<void> {
    ...
    if (last && Date.now() - last < FollowerNotificationService.CHECK_INTERVAL_MS - FollowerNotificationService.DUE_MARGIN_MS) {
  ...
  public async runCheck(): Promise<void> {
    ...
    try {
      ...
      this.systemLogger.error('FollowerNotificationService', `Check failed: ${error}`);
    } finally {
      this.running = false;
    }
  }
