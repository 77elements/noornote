...
export function renderText(block: Extract<Block, { type: 'text' }>, editable = false): string {
  ...
  if (editable) {
    // Composer holds the textarea only. The semantic-tag picker
    // (`p` / `h1`..`h6`) rides in the block-edit toolbar's badge slot
    // via wrapEditable's `badgeOverride`, replacing the static "text"
    // label — mirrors the DIV block's tag-picker placement.
    ...
    `;
    const tagSlot = `<div class="nospress-block-text__tag-slot" data-block-dropdown="text-tag" data-block-id="${block.id}" data-current-value="${escapeHtmlAttr(tag)}"></div>`;
    return wrapEditable(block.id, 'text', inner, linkBtn, tagSlot);
  }
