...
export function wrapEditable(blockId: string, type: string, contentHtml: string): string {
  return `
    <div class="mypage-block-edit" data-block-id="${blockId}" data-block-type="${type}">
      <div class="mypage-block-edit__toolbar">
        ...
        <div class="mypage-block-edit__actions">
          ...
          <button type="button" class="mypage-block-edit__btn" data-block-id="${blockId}" data-action="move-down" title="Move down" aria-label="Move down">
            <svg width="14" height="14"><use href="#icon-chevron-down"/></svg>
          </button>
          <button type="button" class="mypage-block-edit__btn mypage-block-edit__btn--danger" data-block-id="${blockId}" data-action="delete" title="Delete block" aria-label="Delete">
            <svg width="14" height="14"><use href="#icon-close"/></svg>
          </button>
        </div>
      </div>
      <div class="mypage-block-edit__content">${contentHtml}</div>
    </div>
  `;
}
