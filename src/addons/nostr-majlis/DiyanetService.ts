...
export interface DiyanetPlace { id: string; name: string; }
export interface DiyanetDayTimes {
  date: string; // dd.mm.yyyy
  fajr: string; sunrise: string; dhuhr: string; asr: string; maghrib: string; isha: string;
}

interface RawUlke { UlkeID: string; UlkeAdi: string; UlkeAdiEn?: string; }
interface RawSehir { SehirID: string; SehirAdi: string; }
interface RawIlce { IlceID: string; IlceAdi: string; }
interface RawVakit {
...
export class DiyanetService {
  ...
  static getInstance(): DiyanetService {
    if (!DiyanetService.instance) DiyanetService.instance = new DiyanetService();
  ...
  async getRegions(ulkeId: string): Promise<DiyanetPlace[]> {
    ...
    const list = raw.map(s => ({ id: s.SehirID, name: s.SehirAdi })).sort((a, b) => a.name.localeCompare(b.name));
  ...
  async getDistricts(sehirId: string): Promise<DiyanetPlace[]> {
    ...
    const list = raw.map(i => ({ id: i.IlceID, name: i.IlceAdi })).sort((a, b) => a.name.localeCompare(b.name));
  ...
  private readCache(): Record<string, DiyanetDayTimes[]> {
    return PerAccountLocalStorage.getInstance().get<Record<string, DiyanetDayTimes[]>>(
      StorageKeys.NOSTR_MAJLIS_DIYANET_CACHE, {}
    );
  }

  private writeCache(map: Record<string, DiyanetDayTimes[]>): void {
    PerAccountLocalStorage.getInstance().set(StorageKeys.NOSTR_MAJLIS_DIYANET_CACHE, map);
  }
  ...
  async fetchAndCacheTimes(ilceId: string): Promise<DiyanetDayTimes[]> {
    ...
    const fresh: DiyanetDayTimes[] = raw
      ...
      .map(r => ({
        ...
        fajr: r.Imsak, sunrise: r.Gunes, dhuhr: r.Ogle, asr: r.Ikindi, maghrib: r.Aksam, isha: r.Yatsi,
      }));
    ...
    const merged = [...byDate.values()].sort((a, b) => dateKey(a.date) - dateKey(b.date));
    cache[ilceId] = merged;
    this.writeCache(cache);
    diagLog('addons', 'nostr-majlis: Diyanet times cached', { ilceId, fetched: fresh.length, total: merged.length });
    return merged;
  }
