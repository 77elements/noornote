...
export async function loadCityData(): Promise<CityData> {
  ...
  const mod = await import('./data/prayer-cities.json') as unknown as { default: CityData };
  cache = mod.default;
  diagLog('addons', 'nostr-majlis: city dataset loaded', { cities: cache.cities.length });
  return cache;
}
...
export function regionOptions(data: CityData, cc: string): DropdownOption[] {
  ...
  for (const [, ccc, a1] of data.cities) {
    if (ccc !== cc) continue;
    const key = a1 || '';
    if (!seen.has(key)) seen.set(key, key ? (data.regions[`${cc}.${key}`] || key) : '(Other)');
  }
  return [...seen.entries()]
...
export function cityOptions(data: CityData, cc: string, a1: string): DropdownOption[] {
  const out: DropdownOption[] = [];
  data.cities.forEach((c, i) => {
    if (c[1] === cc && (c[2] || '') === a1) out.push({ value: String(i), label: c[0] });
  });
  return out;
}
