...
export class NostrMajlisAddonView extends View {
  ...
  constructor(initialTab?: string) {
    ...
    this.container.className =
      'view-content view-content--addon view-content--addon-nostr-majlis';
  ...
  private render(): void {
    this.enableSwitch = new Switch({
      ...
      onChange: checked => {
        ...
        TypedEventBus.getInstance().emit('nostr-majlis:addon-toggle', {
          enabled: checked,
        });
        ToastService.show(
          checked ? 'Nostr-Majlis enabled' : 'Nostr-Majlis disabled',
          'success'
        );
    ...
    this.calendarSubId = TypedEventBus.getInstance().on(
      'settings:calendar-system-changed',
      () => this.renderHolidaysTable()
    );
    // Live dhikr updates come over the bus, so this works regardless of when the service started.
    this.dhikrBusSub = TypedEventBus.getInstance().on(
      'nostr-majlis:dhikr-changed',
  ...
  private stale(token: number): boolean {
    return (
      this.disposed || token !== this.renderToken || !isNostrMajlisEnabled()
    );
  }

  private renderSalah(): void {
    const slot = this.container.querySelector(
      '[data-addon-content="salah"]'
    ) as HTMLElement | null;
    if (!slot) return;
    ...
    const tabsBar = this.container.querySelector(
      '[data-el="majlis-tabs"]'
    ) as HTMLElement | null;
  ...
  private renderHolidays(): void {
    ...
    `;
    slot
      .querySelector('[data-action="prev-year"]')
      ?.addEventListener('click', () => {
        this.holidayYear -= 1;
        this.renderHolidaysTable();
      });
    slot
      .querySelector('[data-action="next-year"]')
      ?.addEventListener('click', () => {
        this.holidayYear += 1;
        this.renderHolidaysTable();
      });
  ...
  private renderHolidayReminders(): void {
    ...
    this.holidaySwitch = new Switch({
      ...
      onChange: c => {
        ...
        setNostrMajlisSettings({
          ...s,
          holidayReminder: { ...s.holidayReminder, enabled: c },
        });
        this.renderHolidayReminders();
      },
    });
      ...
      this.holidayDaysDD = new CustomDropdown({
        ...
        onChange: v => {
          ...
          setNostrMajlisSettings({
            ...
            holidayReminder: {
              ...s.holidayReminder,
              daysBefore: parseInt(v, 10),
            },
          });
        },
      });
      host
        .querySelector('[data-control="holiday-days"]')
        ?.appendChild(this.holidayDaysDD.getElement());
    }
  }

  private disposeHolidayReminders(): void {
    this.holidayDaysDD?.destroy();
    this.holidayDaysDD = null;
    this.holidaySwitch?.destroy();
  ...
  private renderDhikr(): void {
    const slot = this.container.querySelector(
      '[data-addon-content="dhikr"]'
    ) as HTMLElement | null;
    if (!slot) return;
    this.notifySwitch?.destroy();
  ...
  private bindDhikrActions(
    host: HTMLElement,
    svc: DhikrService,
    isAdmin: boolean
  ): void {
    const roundFor = (btn: Element) =>
      svc
        .getRounds(isAdmin)
        .find(r => r.addr === (btn as HTMLElement).dataset.addr);
    ...
    host.querySelectorAll('[data-action="edit"]').forEach(btn =>
      btn.addEventListener('click', () => {
        const round = roundFor(btn);
        if (round) new DhikrModal('edit', round).open();
      })
    );
    host
      .querySelectorAll('[data-action="hide"]')
      .forEach(btn =>
    ...
    host
      .querySelectorAll('[data-action="unhide"]')
      .forEach(btn =>
        btn.addEventListener(
          'click',
          () =>
            void this.runModeration(() =>
              svc.unhideRound((btn as HTMLElement).dataset.addr ?? '')
            )
        )
      );
    host
      .querySelectorAll('[data-action="ban"]')
      .forEach(btn =>
        btn.addEventListener(
          'click',
          () =>
    ...
    host
      .querySelectorAll('[data-action="unban"]')
      .forEach(btn =>
        btn.addEventListener(
          'click',
          () =>
            void this.runModeration(() =>
              svc.unbanAuthor((btn as HTMLElement).dataset.pubkey ?? '')
            )
        )
      );
  }
  ...
  private async confirmBan(svc: DhikrService, pubkey: string): Promise<void> {
    ...
    if (ok)
      await this.runModeration(() => svc.banAuthor(pubkey), 'Author excluded');
  }
  ...
  private ensureAuthorNames(pubkeys: string[]): void {
    ...
    const missing = pubkeys.filter(
      pk => !profiles.hasProfile(pk) && !this.requestedAuthors.has(pk)
    );
    if (missing.length === 0) return;
    for (const pk of missing) this.requestedAuthors.add(pk);
    void Promise.all(
      missing.map(pk => profiles.getUserProfile(pk).catch(() => null))
    ).then(() => {
      if (!this.disposed) this.renderDhikrList();
    });
  }
  ...
  private renderHolidaysTable(): void {
    ...
    if (!slot) return;
    const yearEl = slot.querySelector(
      '[data-el="holiday-year"]'
    ) as HTMLElement | null;
    const tableEl = slot.querySelector(
      '[data-el="holiday-table"]'
    ) as HTMLElement | null;
    ...
    tableEl.innerHTML = getHolidaysForGregorianYear(this.holidayYear)
      .map(h => {
    ...
    const prev = slot.querySelector(
      '[data-action="prev-year"]'
    ) as HTMLButtonElement | null;
    const next = slot.querySelector(
  ...
  private async renderPanel(): Promise<void> {
    ...
    panel
      .querySelector('[data-control="source"]')
      ?.appendChild(this.sourceDD.getElement());
  ...
  private renderTimes(
    timesEl: HTMLElement,
    t: ComputedTimes | DiyanetDayTimes | null
  ): void {
    const times = t ?? ZERO;
    timesEl.innerHTML = `<div class="ui-list times">${PRAYERS.map(
      ([k, name]) =>
        `<div class="ui-list__item"><span class="setting__label">${name}</span><span>${escapeHtml(times[k])}</span></div>`
    ).join('')}</div>`;
  }

  private renderTimesPrompt(): void {
    const locEl = this.container.querySelector(
      '[data-el="location"]'
    ) as HTMLElement | null;
    const timesEl = this.container.querySelector(
      '[data-el="times"]'
    ) as HTMLElement | null;
  ...
  private async buildDiyanet(): Promise<void> {
    ...
    if (picker)
      picker.innerHTML = `<p class="setting__desc pulsate">Loading…</p>`;

    let countries: DiyanetPlace[];
    try {
      countries = await DiyanetService.getInstance().getCountries();
    } catch {
      if (token === this.renderToken && picker)
        picker.innerHTML = `<p class="setting__desc">Could not reach the Diyanet service. Check your connection and reopen this page.</p>`;
      return;
    }
    ...
    `;
    const actions = this.container.querySelector(
      '[data-el="actions"]'
    ) as HTMLElement | null;
    if (actions)
      actions.innerHTML = `<div class="l-row--right"><button class="btn btn--passive" data-action="fetch">Fetch Prayer Times</button></div>`;
    actions
      ?.querySelector('[data-action="fetch"]')
      ?.addEventListener('click', () => void this.onFetch());

    this.mountCtl(
      'd-country',
      this.optionList(this.dCountries, 'Select country…'),
      this.dUlke,
      true,
      v => void this.onDCountry(v)
    );
    this.mountCtl(
      'd-region',
      this.optionList(this.dRegions, 'Select region…'),
      this.dSehir,
      false,
      v => void this.onDRegion(v)
    );
    this.mountCtl(
      'd-district',
      this.optionList(this.dDistricts, 'Select district…'),
      this.dIlce,
      false,
      v => void this.onDDistrict(v)
    );
  ...
  private async restoreDiyanet(
    ...
    try {
      ...
      this.dDistricts =
        await DiyanetService.getInstance().getDistricts(sehirId);
      if (this.stale(token)) return;
      this.dIlce = ilceId;
      this.mountCtl(
        'd-district',
  ...
  private async onDCountry(ulkeId: string): Promise<void> {
    ...
    this.mountCtl(
      'd-district',
      this.optionList([], 'Select district…'),
      PICK,
      false,
      v => void this.onDDistrict(v)
    );
    ...
    try {
      ...
      ToastService.show('Could not load regions', 'error');
    }
  }
  ...
  private async onDRegion(sehirId: string): Promise<void> {
    ...
    try {
      ...
      ToastService.show('Could not load districts', 'error');
    }
  }
  ...
  private async showDiyanetTimes(ilceId: string, token: number): Promise<void> {
    ...
    if (!today && svc.cachedRows(ilceId).length === 0) {
      ...
      try {
        await svc.fetchAndCacheTimes(ilceId);
      } catch {
    ...
    locEl.textContent = today
      ? `${label} — ${today.date}`
      : `${label} — no times cached, tap “Fetch Prayer Times”`;
    this.renderTimes(timesEl, today);
  }

  private async onFetch(): Promise<void> {
    if (this.dIlce === PICK) {
      ToastService.show('Pick a district first', 'error');
      return;
    }
    ...
    try {
      ...
      if (btn) {
        btn.disabled = false;
        btn.textContent = 'Fetch Prayer Times';
  ...
  private async buildCalc(method: string): Promise<void> {
    ...
    let data: CityData;
    try {
      data = await loadCityData();
    } catch {
      if (token === this.renderToken && picker)
        picker.innerHTML = `<p class="setting__desc">Could not load city data.</p>`;
      return;
    }
    if (this.stale(token) || !this.picker()) return;
  ...
  private mountGRegion(method: string): void {
    ...
    if (this.cityData && this.gCc !== PICK)
      opts.push(...regionOptions(this.cityData, this.gCc));
    this.mountCtl('g-region', opts, this.gA1, false, v =>
      this.onGRegion(method, v)
    );
  }
  ...
  private mountGCity(method: string): void {
    ...
    if (ready) {
      ...
      if (saved && saved.cc === this.gCc && (saved.a1 || '') === this.gA1) {
        const idx = this.cityData!.cities.findIndex(
          c =>
            c[1] === saved.cc &&
            (c[2] || '') === this.gA1 &&
            c[0] === saved.name &&
    ...
    this.mountCtl('g-city', opts, selected, !!ready, v =>
      this.onGCity(method, v)
    );
  }
  ...
  private mountGMadhab(method: string): void {
    this.mountCtl(
      'g-madhab',
      MADHAB_OPTIONS,
      getNostrMajlisSettings().madhab,
      false,
      v => {
        setNostrMajlisSettings({
          ...getNostrMajlisSettings(),
          madhab: v === 'hanafi' ? 'hanafi' : 'shafi',
        });
        this.updateCalcTimes(method);
      }
    );
  ...
  private updateCalcTimes(method: string): void {
    ...
    const s = getNostrMajlisSettings();
    if (!s.calcCity) {
      locEl.textContent = 'Select your city to see prayer times.';
      timesEl.innerHTML = '';
      return;
    }
  ...
  private renderReminders(): void {
    ...
    this.masterSwitch = new Switch({
      ...
      onChange: c => {
        ...
        setNostrMajlisSettings({
          ...s,
          reminders: { ...s.reminders, enabled: c },
        });
        this.renderReminders();
      },
    });
        ...
        const sw = new Switch({
          ...
          onChange: c => {
            ...
            setNostrMajlisSettings({
              ...
              reminders: {
                ...s.reminders,
                prayers: { ...s.reminders.prayers, [key]: c },
              },
            });
          },
        });
    ...
    if (r.enabled) {
      this.offsetDD = new CustomDropdown({
        ...
        onChange: v => {
          ...
          setNostrMajlisSettings({
            ...s,
            reminders: { ...s.reminders, offsetMin: parseInt(v, 10) },
          });
        },
      });
      host
        .querySelector('[data-control="offset"]')
        ?.appendChild(this.offsetDD.getElement());
      for (const sw of this.praySwitches) sw.setupEventListeners(host);
    }
  }

  private disposeReminders(): void {
    this.offsetDD?.destroy();
    this.offsetDD = null;
    this.masterSwitch?.destroy();
    this.masterSwitch = null;
  ...
  public destroy(): void {
    this.disposed = true;
    if (this.calendarSubId) {
      TypedEventBus.getInstance().off(this.calendarSubId);
      this.calendarSubId = null;
    }
    if (this.dhikrBusSub) {
      TypedEventBus.getInstance().off(this.dhikrBusSub);
      this.dhikrBusSub = null;
    }
    this.disposeDropdowns();
    this.disposeReminders();
    this.disposeHolidayReminders();
